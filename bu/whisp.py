...
import json
import time
# Removed tempfile as we use a fixed temp filename now
import re
...
import numpy as np
import io
# Removed base64 as it's not needed for the transcription endpoint file upload
import traceback
...
RECORD_SECONDS = 5
TTS_VOICE = "onyx"
TTS_MODEL = "tts-1" # Use standard TTS model for potentially lower latency
...
# --- Helper Functions ---
def sanitize_filename(name): # ... (keep as is)
    sanitized = re.sub(r'[\\/*?:"<>|]', "", name)
    sanitized = sanitized.replace(" ", "_")
    return sanitized[:100]

def get_chord_notes(chord_name, base_octave=4): # ... (keep robust version)
    # ... (Existing robust chord parsing logic) ...
...
def play_music_with_pygame(music_params): # ... (keep robust version)
    # ... (Exactly the same as previous version) ...
    ...
    try:
        ...
        if generated_notes_count == 0: error_msg = "Error: Failed to generate any valid notes."; 
        if skipped_chords: error_msg += f" Skipped chords: {', '.join(skipped_chords)}"; print(error_msg); return error_msg
        ...
        try:
            ...
            pygame.mixer.music.load(filepath); print(f"Playing MIDI file: {filepath}..."); pygame.mixer.music.play()
            success_msg = f"Successfully initiated MIDI playback of '{filename}'.";
            if skipped_chords: success_msg += f" Warning: Skipped unparsable chords: {', '.join(skipped_chords)}."; print(f"Warning: Skipped chords {skipped_chords}")
            return success_msg
...
def speak(text_to_speak):
    """Uses OpenAI TTS API and Pygame (mixer channel 0) to speak the text."""
    ...
    try:
        response = openai_client.audio.speech.create(
            model=TTS_MODEL, voice=TTS_VOICE, input=text_to_speak, response_format="mp3") # Using mp3 for broad compatibility
...
# --- STT using sounddevice and OpenAI Transcription API ---
def listen_and_transcribe():
    """Records audio, saves temp WAV, sends to OpenAI Transcription API (Whisper)."""
    # (Device checking logic - unchanged)
    try:
        devices = sd.query_devices(); input_devices = [d for d in devices if d['max_input_channels'] > 0]
        if not input_devices: print("No input audio devices found."); return None
    except Exception as e: print(f"Error querying audio devices: {e}"); return None

    try:
        print(f"\nListening for {RECORD_SECONDS} seconds... (Speak clearly)")
        recording = sd.rec(int(RECORD_SECONDS * SAMPLE_RATE), samplerate=SAMPLE_RATE, channels=1, dtype='int16')
        sd.wait()
        print("Processing audio...")

        # Use BytesIO to avoid writing to disk if possible, pass directly to API
        wav_bytes = io.BytesIO()
        sf.write(wav_bytes, recording, SAMPLE_RATE, format='WAV', subtype='PCM_16')
        wav_bytes.seek(0) # Reset stream position to the beginning

        # *** CORRECTED API CALL ***
        # Pass the file-like object directly to the API client
        response = openai_client.audio.transcriptions.create(
            model=STT_MODEL, # "whisper-1"
            file=(RECORDING_FILENAME, wav_bytes, "audio/wav") # Provide filename tuple for API
            # language="en" # Optional: Specify language if needed
        )

        transcription = response.text # Response object directly contains 'text'
        print(f"Transcription: '{transcription}'")
        return transcription.strip() if transcription else None

    except sd.PortAudioError as pa_err: print(f"Sounddevice/PortAudio Error: {pa_err}. Check mic."); return None
    except openai.APIError as api_err: print(f"OpenAI Transcription API Error: {api_err}"); return None
    except Exception as e: print(f"Error during listening/transcription: {e}"); traceback.print_exc(); return None
    finally:
        # Clean up BytesIO stream
        if 'wav_bytes' in locals() and wav_bytes:
            wav_bytes.close()
        # No physical temp file to delete if BytesIO was used successfully


# --- OpenAI Interaction & Tool Handling ---
conversation_history = [ {"role": "system", "content": """You are an expert AI Music Assistant using OpenAI audio. Interact via voice.
        ...
        3. **Response (Text):** Formulate your response *text*. If calling the tool, include conversational text *before* the call. After tool execution results, generate a final confirmation text acknowledging the tool's feedback.
        4. **Output (Speech):** Your text responses will be converted to speech using OpenAI TTS.
...
def process_user_input_and_respond(user_input_text):
    ...
    try:
        ...
        if tool_calls:
            if ai_response_text_before_tool: speak(ai_response_text_before_tool)
            ...
            second_response = openai_client.chat.completions.create(model=LLM_MODEL, messages=conversation_history)
...
# --- Main Loop ---
print(f"\nAI Music Assistant (OpenAI Voice v2) Initialized. MIDI files saved in '{SONGS_FOLDER}'.")
try:
    ...
    speak("AI Music Assistant ready. How can I help?")
...
while True:
    try:
        user_text = listen_and_transcribe() # Record -> Transcribe via Whisper API
        ...
        if user_text:
             ...
             ai_final_text = process_user_input_and_respond(user_text) # Process -> Opt Tool -> Final Text

             if ai_final_text:
                 speak(ai_final_text) # Speak Final Text via OpenAI TTS

        else:
            print("(No input received or recognized)")

    except (EOFError, KeyboardInterrupt): print("\nExiting..."); speak("Exiting."); break
    except Exception as loop_err: print(f"ERROR in main loop: {loop_err}"); traceback.print_exc(); speak("Sorry, something went wrong."); time.sleep(1)
...
# No physical temp file to delete if BytesIO worked
print("Exited.")