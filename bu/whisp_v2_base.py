...
TTS_VOICE = "nova"  # Using more expressive voice (nova instead of onyx)
TTS_MODEL = "tts-1-hd"  # Use HD model for better quality
TTS_SPEED = 1.2  # Slightly faster speech (1.0 is normal)
...
def speak(text_to_speak):
    ...
    try:
        response = openai_client.audio.speech.create(
            ...
            voice=TTS_VOICE, 
            input=text_to_speak, 
            speed=TTS_SPEED,  # Add speed parameter for faster speech
            response_format="mp3"  # Using mp3 for broad compatibility
        )
...
# --- Main Loop ---
print(f"\nAI Music Assistant (OpenAI Voice v3) Initialized. MIDI files saved in '{SONGS_FOLDER}'.")
try:
    ...
    speak("AI Music Assistant ready with enhanced voice. How can I help create music today?")
    ...
    try:
        ...
        if user_text:
             if user_text.lower() in ['quit', 'exit', 'stop', 'goodbye']:
                 speak("Goodbye! It was a pleasure making music with you.")
                 break
